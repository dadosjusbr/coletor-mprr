...
func (c crawler) abreCaixaDialogo(ctx context.Context, tipo string) error {
	...
	if tipo == "contra"{
		baseURL = "https://www.mprr.mp.br/web/transparencia/opcoesvencimentos?quadro=1"
		selectYear = `//*[@id="ano"]`
		...
		return chromedp.Run(ctx,
			...
			// Seleciona mes
			chromedp.SetValue(`//*[@id="mes"]`, c.month, chromedp.BySearch, chromedp.NodeVisible),
			chromedp.Sleep(c.timeBetweenSteps),
	
			// CAPTCHA
			chromedp.Click(`/html/body/div[2]/div[3]/div[1]/div/div/span`, chromedp.BySearch),
			chromedp.Sleep(c.timeBetweenSteps),

			// Busca
			chromedp.Click(`//*[@id="tab6"]/div/div/div/div/div/div/div[2]/form/button`, chromedp.BySearch, chromedp.NodeVisible),
			chromedp.Sleep(c.timeBetweenSteps),
